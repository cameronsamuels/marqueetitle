var game = {
  on: false, time: 10, score: 0, finished: true,
...
game.over = function() {
  ...
  game.finished = true; //set variable to be over
...
game.start = function() {
  ...
  game.finished = false; //set game to be not over
  game.interval = setInterval(game.refresh, 1000); //set repeating to every 1 second to call game.refresh()
},
game.click = function() {
  if (game.finished == true) game.start(); //start game if game is over
  if (game.on == false) return; //stop execution if paused
